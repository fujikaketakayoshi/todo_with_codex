...
TODO の追加・編集・完了状態の切り替え・削除、ハッシュタグによる自動タグ登録、タグ別（タグなしを含む）の絞り込み表示に対応したシンプルなアプリです。SQLite データベースは初回アクセス時に `data/todos.sqlite` として自動作成されます。既存のカテゴリデータは初回起動時にタグへ自動移行されます。
